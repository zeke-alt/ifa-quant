...
if (!PUBLIC_KEY || !SECRET_KEY) {
  console.error("CRITICAL CONFIG ERROR: BAYSE_PUBLIC_KEY or BAYSE_SECRET_KEY is not defined in environment variables.");
}
...
function createSignature(timestamp: number, method: string, path: string, body?: string) {
  ...
  if (!SECRET_KEY) {
    throw new Error("BAYSE_SECRET_KEY is required for signed requests. Check environment variables.");
  }
 ...
 */
export async function bayseWrite(method: string, path: string, body: object, retries = 3) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        console.warn(`Bayse Write ${res.status} (Attempt ${i + 1}), retrying...`);
        await new Promise(r => setTimeout(r, 1000 * (i + 1)));
        continue;
      }
      ...
      lastError = err;
      console.error(`Bayse Write attempt ${i + 1} failed:`, err.message);
      if (i < retries - 1) {
        await new Promise(r => setTimeout(r, 1000 * (i + 1)));
      }
    }
